...
from ..utils import (check_table_exists, fetch_all, check_column_exists,
                     compare_files)
...
def test_create_new_migration(monkeypatch, migration_dir, capsys):
    ...
    monkeypatch.setattr('padmy.migration.utils.get_git_email',
                        lambda x: TEST_EMAIL)
    ...
    new_files = list(migration_dir.glob('*.sql'))
    assert capsys.readouterr().out.strip().startswith('Creating new migration file')
    down_file2, up_file2 = sorted(new_files, key=lambda x: x.name.split('-')[0],
                                  reverse=True)[:2]
...
def test_migrate_verify_invalid(monkeypatch, engine, tmp_path):
    ...
    with pytest.raises(MigrationError,
                       match=re.escape('Difference found for migration: 00000000')) as e:
        migrate_verify(database=PG_DATABASE,
                       schemas=['general'],
                       dump_dir=tmp_path,
    ...
    assert compare_files(error_file, INVALID_MIGRATIONS_DIR / '1-00000000.diff')


SETUP_ERROR_MSG = re.escape('Could not find table table "public.migration", '
...
def test_migrate_up_no_setup(engine, monkeypatch, aengine, loop):
    ...
    async def _test():
        with pytest.raises(NoSetupTableError,
                           match=SETUP_ERROR_MSG):
...
@pytest.mark.usefixtures('clean_migration')
def test_migrate_down_no_setup(engine, monkeypatch, caplog, aengine,
                               loop):
...
def test_migrate_up_down(engine, monkeypatch, caplog,
                         aengine, loop):
    ...
    # 1rst migration
    loop.run_until_complete(migrate_up(aengine,
                                       folder=VALID_MIGRATIONS_DIR,
                                       nb_migrations=1))
